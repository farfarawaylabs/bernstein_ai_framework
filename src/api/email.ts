...
app.post("/incoming", async (c) => {
	...
	if (typeof from === "string") {
		...
		if (email) {
			...
			const operator = new Operator({ tools: {} });

			const conductor = new Conductor({
				operator: operator,
				defaultModel: AI_MODELS.CHATGPT4O,
				stateSerializer: new KVStoreConversationSerializer(
					60 * 60 * 24 * 1,
				),
				conversationId: conversationId!,
				hooks: {
					onConversationStopped: async (
			...
			const lastToolCall = conductor.conversation?.getLastToolCalls()
				.find((toolCall) => toolCall.name === "ask_user_input");

			conductor.addUserInputToConversation(
				extractUserReply(emailBody as string),
				lastToolCall!.id!,
			);
